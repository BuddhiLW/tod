...
(defun tod-sun-utc-midnight (date)
  "Return the Lisp time of 00:00 UTC on DATE, a (MONTH DAY YEAR) list."
  (encode-time (list 0 0 0 (clel-nth date 1) (clel-nth date 0) (clel-nth date 2) nil nil t)))
...
(defun tod-sun-crossings (date latitude longitude height)
  ...
  (let* ((calendar-latitude latitude)
        ...
        (noon (solar-exact-local-noon date))
...
(defun tod-sun--day-events (date latitude longitude heights)
  "Return the crossing events of DATE for every height in HEIGHTS.\nEach event is an alist with :time, :height and :rising.  LATITUDE and\nLONGITUDE are in degrees."
  (mapcan (lambda (height)
    (let* ((pair (tod-sun-crossings date latitude longitude height)))
    (delq nil (list (when (clel-first pair)
    (list (cons :time (clel-first pair)) (cons :height height) (cons :rising t))) (when (clel-second pair)
    (list (cons :time (clel-second pair)) (cons :height height) (cons :rising nil))))))) heights))
...
(defun tod-sun-events-around (time latitude longitude heights)
  ...
  (let* ((dates (list (tod-sun-utc-date (time-subtract time 86400)) (tod-sun-utc-date time) (tod-sun-utc-date (time-add time 86400))))
        (all (mapcan (lambda (date)
    (tod-sun--day-events date latitude longitude heights)) dates)))

...
(defvar tod-palette-semantic-hues
    (list (list :red 0.0)
          (list :orange 30.0)
          (list :yellow 55.0)
          (list :green 120.0)
...
Slots 0, 7, 8 and 15 hold the background, foreground and greys."
  (let* ((colors (clel-get wal :colors)))
    (seq-filter
     (lambda (c)
       (and (tod-color-hex-to-rgb c) (>= (tod-color-saturation c) 0.15)))
     (mapcar (lambda (i) (clel-nth colors i))
...
Colours more than 40 degrees away do not count."
  (let* ((scored
          (mapcar
           (lambda (c)
             (cons (tod-color-hue-distance (tod-color-hue-degrees c) hue) c))
           candidates))
         (best (car (clel-sort (lambda (a b) (< (car a) (car b))) scored))))
    (when (and best (<= (car best) 40.0)) (cdr best))))
...
WARMTH of the warm light mixed in, other phases none."
  (if (and (memq phase (list 'golden-hour 'civil-twilight)) (> warmth 0.0))
      (cons
       (cons
        :background (tod-color-mix (clel-get wal :background)
                                   tod-palette-warm-light
                                   warmth))
       wal)
    wal))
...
in `tod-palette-semantic-hues'."
  (let* ((bg (or (clel-get wal :background) "#000000"))
         (fg
          (tod-color-ensure-contrast
           (or (clel-get wal :foreground)
               (if (tod-color-dark-p bg) "#ffffff" "#000000"))
           bg
           (max ratio 7.0)))
          ...
          (mapcar
           (lambda (entry)
             (let* ((hue (clel-second entry))
                    (base
                     (or (tod-palette-nearest-accent candidates hue)
                         (tod-color-from-hsl (/ hue 360.0) 0.55 0.5))))
               (cons (clel-first entry)
                     (tod-color-ensure-contrast base bg ratio))))
           tod-palette-semantic-hues))
         (grey
          (or (clel-nth (clel-get wal :colors) 8) (tod-color-mix fg bg 0.45))))
    (append
     (list (cons :bg bg)
           (cons :fg fg)
           (cons
            :cursor (tod-color-ensure-contrast (or (clel-get wal :cursor) fg)
                                               bg
                                               3.0))
           (cons :comment (tod-color-ensure-contrast grey bg ratio)))
     accents)))
...
terminals and other programs; otherwise it writes under CACHE-DIR."
  (if system
      (expand-file-name "wal/colors.json"
...
programs; when non-nil it recolours the desktop as pywal usually does."
  (let* ((flags
          (append (list "-i" (expand-file-name image) "-n" "-q")
                  (when light (list "-l")))))
    (if system
      ...
      (list
       (append
        (list "env"
              (clel-str "PYWAL_CACHE_DIR=" (expand-file-name "wal" cache-dir))
              "NO_FUN=1"
              "wal")
        flags
        (list "-s" "-t" "-e"))))))
...
`tod-palette-wal-commands'."
  (let* ((out (tod-palette-wal-colors-file cache-dir system)))
    (tod-process-run 'palette
                     (tod-palette-wal-commands image cache-dir light system)
                     (lambda (ok message)
                       (let* ((wal (when ok (tod-palette-read-wal out))))
                         (funcall
                          done
                          wal
                          (cond
                           ((not ok) message)
                           ((null wal)
                            (clel-str "pywal wrote no palette to " out))
                           (t nil))))))))
